...
const getDimensions = (resolution, aspectRatio) => {
    let width = 1920;
    let height = 1080;
    switch (resolution) {
        case '360p':
...
export async function generateVideo(audioPath, state) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const drawText = (text, xPct, yPct, style, baseSizeRef, scaleFactor) => {
            if (!text)
                return;
